...
def cdn_1():
    ...
    for data in collection.find({'flag':0,'id_count':{'$gt':0,'$lt':250000}}):
        ...
        dns_record = [{'A_record':A_record_handle(a_record),'CNAME_record':cname_record,'NS_record':NS_record_handle(ns_record),'insert_time':datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]
